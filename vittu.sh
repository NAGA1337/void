...
mkfs.$FSYS -L root /dev/nvme0n1p3
...
$(blkid ${DEVNAME}p1 | cut -d ' ' -f 4 | tr -d '"') /boot vfat  rw,noatime,discard  0 2
$(blkid ${DEVNAME}p2 | cut -d ' ' -f 3 | tr -d '"') swap  swap  rw,noatime,discard  0 0
$(blkid ${DEVNAME}p3 | cut -d ' ' -f 3 | tr -d '"') / ext4 rw,noatime,discard 0 1
EOF
